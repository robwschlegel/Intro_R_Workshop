...
dune_MDS <- metaMDS(dune, autotransform = F)
```

Or we may be more specific in the way in which we prepare our data for the MDS.
...
```{r multi-plot, fig.cap="The basic biplot of our dune results."}
# Convert for ggplot
dune_MDS_points <- data.frame(site = 1:nrow(dune)) %>%
  mutate(x = as.numeric(dune_MDS$points[ ,1]),
         y = as.numeric(dune_MDS$points[ ,2]))
...
```{r multi-env}
dune_envfit <- envfit(dune_MDS ~ Moisture + Use, data = dune.env)
dune_envfit
```
